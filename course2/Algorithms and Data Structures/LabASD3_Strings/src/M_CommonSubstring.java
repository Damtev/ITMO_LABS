...
public class M_CommonSubstring {
    ...
    public static void main(String[] args) throws IOException {
        in = new StreamTokenizer(new FileReader("common.in"));
        k = 2;
        ...
        try (PrintWriter pr = new PrintWriter(new FileWriter("common.out"))) {
            if (k == 1) {
                pr.println(strings[0]);
            } else {
                calcPow();
                hashes = new long[k][];
                for (int i = 0; i < k; i++) {
                ...
                if (subSize > 0) {
                    ...
                    pr.println(strings[0].substring(left, left + subSize));
                } else {
                    pr.println();
                }
            }
