...
public class H_MultipleSearch_2 {
    ...
    private static void read() throws IOException {
        in = new StreamTokenizer(new FileReader("search5.in"));
        n = nextInt();
        for (int i = 0; i < n; i++) {
    ...
    private static void printAnswers() throws IOException {
        try (PrintWriter pr = new PrintWriter(new FileWriter("search5.out"))) {
            StringBuilder answer = new StringBuilder();
            for (String pattern : ahoCorasick.patterns) {
                answer.append(ahoCorasick.positions.get(pattern).size()).append("\n");
            }
